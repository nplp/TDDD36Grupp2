...
def reconnect():
    ...
    clientSocket.close()
    try:
	subprocess.call('ssh -f nikpe890@'+HOST2+' -L'+str(MYPORT2)+':127.0.0.1:'+str(PORT2)+' sleep 4', shell=True)
    except error:
	print 'no server baby i reconnect'
    print "baddap"
    clientSocket2.connect(ADDR2)
    print "baddap2"
    recThread2 = recieverClass(clientSocket2, ADDR2)
    print "baddap3"
    recThread2.start()
    print "baddap4"
